...
def train(cfg: TrainCfg):
    ...
    for ep in range(1, cfg.epochs + 1):
        ...
        for A, B, pathsA, pathsB in dl_tr:
            ...
            if (cfg.log_every > 0) and (global_step % cfg.log_every == 0):
                ...
                print(
                    ...
                    f"Lrec {avg(q_lrec):.4f} Lgrad {avg(q_lgrad):.4f} Llat {avg(q_llat):.5f} Lzcal {avg(q_lzcal):.5f} | "
        ...
        va_rec /= max(1, va_n)
        va_grad /= max(1, va_n)
        print(f"[ep {ep:02d}]   val: Lrec={va_rec:.4f}  Lgrad={va_grad:.4f}")
