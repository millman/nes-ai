...
class SimpleScreenRxC:
    ...
    def blit_image_np(self, image_np: NdArrayRGB8, screen_index: int):
        # print(f"SURF SIZE: {self.surfs[screen_index].get_size()}  image_np.shape={image_np.shape}")

        image_np_wh = image_np.swapaxes(0, 1)
        pygame.surfarray.blit_array(self.surfs[screen_index], image_np_wh)
    ...
    def blit_image(self, image: Image, screen_index: int):
        ...
        # np.asarray(image) is (height, width), so we swap axes to get back to (width, height).
        image_np_wh = np.asarray(image).swapaxes(0, 1)
        pygame.surfarray.blit_array(self.surfs[screen_index], image_np_wh)
