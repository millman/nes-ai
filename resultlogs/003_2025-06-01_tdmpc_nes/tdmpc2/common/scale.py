...
class RunningScale(torch.nn.Module):
	...
	def __init__(self, cfg, device: str):
		super().__init__()
		self.cfg = cfg
		self.value = Buffer(torch.ones(1, dtype=torch.float32, device=torch.device(device)))
		self._percentiles = Buffer(torch.tensor([5, 95], dtype=torch.float32, device=torch.device(device)))
