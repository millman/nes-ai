...
# -*- coding: utf-8 -*-
"""
Action-distance reconstruction trainer with a ResNet-style autoencoder and
full A→B latent interpolation visualization with trajectory/state labels.

Run:
  python action_distance_recon_resnet.py --data_root traj_dumps --out_dir out.action_distance_recon_resnet
"""
...
class ResBlock(nn.Module):
    """Basic residual block used by the ResNet encoder."""
...
class Encoder(nn.Module):
    """ResNet-style encoder that downsamples NES frames by 16x."""
...
class ResUpBlock(nn.Module):
    """Residual upsampling block mirroring the encoder structure."""
...
class Decoder(nn.Module):
    """Symmetric ResNet-style decoder with sigmoid output."""
    ...
    def __init__(self, z_dim: int = 64, *, use_refine: bool, self_refine_mids: bool, refine_ch: int = 64):
        ...
        self.fc = nn.Linear(z_dim, 512 * 15 * 14)
        ...
        self.head = nn.Conv2d(32, 3, kernel_size=3, padding=1)
...
class TrainCfg:
    data_root: Path
    out_dir: Path = Path("out.action_distance_recon_resnet")
    z_dim: int = 64
    batch_size: int = 16
    epochs: int = 20
    lr: float = 1e-3
