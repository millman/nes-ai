...
- pixel_delta
- inverse_dynamics_z, inverse_dynamics_p
- action_delta_z
- additivity_h
...
- `ModelConfig.pose_correction_use_z = False`
  - no z-based correction into pose by default.
- `ModelConfig.pose_correction_detach_z = True`
  - if correction is enabled later, it is stop-grad on z by default.
                      ...
                      \                         (recon_multi_box, pixel_delta_multi_box)
                       \-- stop-grad? --------> Predictor(h_t, a_t) ---> h_{t+1}
                                                ...
                                                +--> p_action_delta_projector ---> pose_t
                                                     ...
                                                     (action_delta_p, additivity_p, rollout_kstep_p, scale_p)
                                                     +--> geometry_rank_p
```
...
| action_delta_p | pose_pred vs pose_obs | p_action_delta_projector | `pose_delta_detach_h=True` blocks grads into h. |
| additivity_p | delta composition | p_action_delta_projector | Same as above. |
| rollout_kstep_p | k-step integration | p_action_delta_projector | Same as above. |
| scale_p | pose stats vs target | p_action_delta_projector | Same as above. |
...
- **p (planning pose)**: odometry algebra + planning geometry are active.
  - `action_delta_p`, `additivity_p`, `rollout_kstep_p` are the algebraic constraints.
  - `scale_p` anchors magnitude; `geometry_rank_p` adds goal-conditioned ordering.
